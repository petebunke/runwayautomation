  ...
  const pollTaskCompletion = async (taskId, jobId, promptText, imageUrlText, jobIndex) => {
    ...
    while (pollCount < maxPolls) {
      try {
        ...
        // Handle PENDING status with timeout detection
        if (task.status === 'PENDING') {
          if (!isPending) {
            isPending = true;
            pendingStartTime = Date.now();
            addLog('⏳ Job ' + (jobIndex + 1) + ' is pending - waiting for processing to begin...', 'info');
          }
          
          const pendingDuration = Math.floor((Date.now() - pendingStartTime) / 1000);
          
          // If pending for more than 10 minutes, consider it stuck
          if (pendingDuration > 600) {
            const errorMsg = 'Job stuck in pending status for over 10 minutes. This may be a RunwayML API issue.';
            addLog('⚠️ Job ' + (jobIndex + 1) + ' ' + errorMsg, 'warning');
            setGenerationProgress(prev => ({
              ...prev,
              [jobId]: { 
                status: 'stuck', 
                progress: 15,
                error: errorMsg,
                message: `Stuck pending for ${Math.floor(pendingDuration / 60)}m` 
              }
            }));
            
            // Continue polling but warn user
            if (pendingDuration % 120 === 0) { // Every 2 minutes
              addLog('⚠️ Job ' + (jobIndex + 1) + ' still stuck pending after ' + Math.floor(pendingDuration / 60) + ' minutes. Consider stopping and retrying.', 'warning');
            }
          }
          ...
          setGenerationProgress(prev => ({
            ...prev,
            [jobId]: { 
              status: pendingDuration > 600 ? 'stuck' : 'pending', 
              progress: 15,
              message: `Pending for ${pendingDuration}s` 
            }
          }));
          
          if (pendingDuration > 0 && pendingDuration % 60 === 0 && pendingDuration <= 600) {
            addLog('⏳ Job ' + (jobIndex + 1) + ' still pending after ' + Math.floor(pendingDuration / 60) + ' minute(s)', 'info');
          }
        } else if (isPending && task.status !== 'PENDING') {
          const pendingTime = Math.floor((Date.now() - pendingStartTime) / 1000);
          addLog('▶️ Job ' + (jobIndex + 1) + ' moved from pending to ' + task.status.toLowerCase() + ' after ' + pendingTime + 's', 'info');
          isPending = false;
        }
