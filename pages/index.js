...
export default function RunwayAutomationApp() {
  ...
  const checkForProfanity = (text) => {
    ...
    // Check for profanity within the full text (handles concatenated words)
  ...
  const handlePromptChange = (e) => {
    ...
    if (containsProfanity && !promptContainsProfanity) {
      ...
      return; // Don't update the prompt until user acknowledges
    }
    
    setPrompt(newPrompt);
    
    // Reset profanity flag if content is now clean
    if (!containsProfanity && promptContainsProfanity) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedPrompt && savedPrompt.trim()) {
        ...
        // Check for profanity in saved prompt without showing modal
